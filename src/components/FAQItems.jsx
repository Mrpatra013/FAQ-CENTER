...
const FAQItems = ({ item, onClick, isOpen }) => {
  ...
  return (
    <div
      className="border-b border-gray-299 dark:border-gray-700 last:border-none transition-colors duration-300  hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-transparent dark:from-gray-800/50 dark:to-transparent cursor-pointer"
      id={`faq-item-${item.id}`}
    >
      <button
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-center w-8 min-w-8 aspect-square rounded-full bg-gray-200 dark:bg-gray-700 transition-all duration-200">
          <i className={`bx ${isOpen ? 'bx-minus' : 'bx-plus'} text-gray-500 dark:text-gray-400`}></i>
        </div>
      </button>
      <div
